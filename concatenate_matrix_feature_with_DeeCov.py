...
def plot(a):
	pdb_name_file="pdb_3_list_temp.txt"
...
#		chains=ff.readline()
		...
		matrix_DeepCov_A_B_name=os.path.join('./2_chain_DeepCov/',pdb_name+'_'+chain_A+'_'+chain_B+'.matrix')
		matrix_DeepCov_A_C_name=os.path.join('./2_chain_DeepCov/',pdb_name+'_'+chain_A+'_'+chain_C+'.matrix')
		matrix_DeepCov_B_C_name=os.path.join('./2_chain_DeepCov/',pdb_name+'_'+chain_B+'_'+chain_C+'.matrix')
		...
		matrix_DeepCov_A_B=np.loadtxt(matrix_DeepCov_A_B_name)
		matrix_DeepCov_A_C=np.loadtxt(matrix_DeepCov_A_C_name)
		matrix_DeepCov_B_C=np.loadtxt(matrix_DeepCov_B_C_name)
		...
		if l_A == l_A_1 and l_A == l_A_2 and ratio_A>0.05:
			if l_B == l_B_1 and l_B == l_B_2 and ratio_B>0.05:
				if l_C == l_C_1 and l_C == l_C_2 and ratio_C>0.05:
					...
					temp_1=range(0,l_A)
					temp_2=range(l_A,l_A+l_B)
					temp_3=matrix_DeepCov_A_B[temp_1]
					DeepCov_A_B=temp_3[:,temp_2]				
					random_A_B=DeepCov_A_B
					...
					feature_A_B_name=os.path.join('./2_chain_feature_with_DeepCov/',pdb_name+'_'+chain_A+'_'+chain_B+'.jpg')
...
#					random_A_C=np.random.random((l_A,l_C))
					temp_1=range(0,l_A)
                                        temp_2=range(l_A,l_A+l_C)
                                        temp_3=matrix_DeepCov_A_C[temp_1]
                                        DeepCov_A_C=temp_3[:,temp_2]
					random_A_C=DeepCov_A_C
			                ...
			                feature_A_C_name=os.path.join('./2_chain_feature_with_DeepCov/',pdb_name+'_'+chain_A+'_'+chain_C+'.jpg')
...
#					random_B_C=np.random.random((l_B,l_C))
					temp_1=range(0,l_B)
                                        temp_2=range(l_B,l_B+l_C)
                                        temp_3=matrix_DeepCov_B_C[temp_1]
                                        DeepCov_B_C=temp_3[:,temp_2]
					random_B_C=DeepCov_B_C
			                ...
			                feature_B_C_name=os.path.join('./2_chain_feature_with_DeepCov/',pdb_name+'_'+chain_B+'_'+chain_C+'.jpg')
