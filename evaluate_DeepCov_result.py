...
DeepCov_acc=[]
j=0
for filename in os.listdir('./CM_IDF/val'):
	temp=filename.split('.')
	name=temp[0]
	...
	predict_matrix_file=os.path.join('./TEST.1.11/images/',name+'-outputs.png')
#	ccmpred_matrix=os.path.join('./TEST.1.7/images/',name+'-inputs.png')
	contact_matrix_file=os.path.join('./TEST.1.11/images/',name+'-targets.png')
