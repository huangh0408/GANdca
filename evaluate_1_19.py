...
sub_acc=[]
j=0
for filename in os.listdir('./cm_benchmark_1_19'):
	temp=filename.split('.')
	name=temp[0]
...
#	L=temp_matrix.shape[1]
	DeepCov_matrix_name=os.path.join('./benchmark_DeepCov_matrix/',name+'.matrix')
...
#	temp_matrix_file=os.path.join('/home/huanghe/workspace_1_2/2_chain_flag_matrix/',name+'.txt')
	true_matrix_file=os.path.join('./hh_1_19/images/',name+'-targets.png')	
	predict_matrix_file=os.path.join('./hh_1_19/images/',name+'-outputs.png')
...
#	contact_matrix_file=os.path.join('./hh_1_14/images/',name+'-targets.png')
	temp_matrix=np.loadtxt(DeepCov_matrix_name)
	L=temp_matrix.shape[1]
	predict_matrix_file_gray=Image.open(predict_matrix_file).convert('L')
	true_matrix_file_gray=Image.open(true_matrix_file).convert('L')
...
#	input_acc.append(evaluate_temp(DeepCov_matrix_temp, true_matrix))
#	sub_acc.append(evaluate_temp(sub_predict_A_B_matrix, sub_temp_matrix))
	output_acc.append(evaluate(predict_matrix,true_matrix))
	print "\n"
	print "*"*50
	print "\nAcc result:%s" %name
	print "*"*50
